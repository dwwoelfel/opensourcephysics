...
public class DatasetCurveFitter extends JPanel {
  ...
  public double fit(KnownFunction fit) {
    ...
    if (showCorrelation) {
      double rSquared = getPearsonCorrelationSquared(fit, x, y);
      rmsField.applyPattern("0.000"); //$NON-NLS-1$
      rmsField.setValue(rSquared);
    }
    else {
  ...
  protected void createGUI() {
    ...
    rmsLabel.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent e) {
      	JPopupMenu popup = new JPopupMenu();
        JMenuItem rmsItem = new JMenuItem(
        		ToolsRes.getString("DatasetCurveFitter.MenuItem.RMSDeviation")); //$NON-NLS-1$
        popup.add(rmsItem);
        final JMenuItem corrItem = new JMenuItem(
        		ToolsRes.getString("DatasetCurveFitter.MenuItem.CorrelationCoefficient")); //$NON-NLS-1$
        popup.add(corrItem);
        ActionListener action = new ActionListener() {
          public void actionPerformed(ActionEvent e) {
          	showCorrelation = e.getSource()==corrItem;
          	refreshGUI();
          }
        };
        rmsItem.addActionListener(action);
        corrItem.addActionListener(action);
        popup.show(rmsLabel, 0, rmsLabel.getHeight());
      }
...
//        fitBuilder.setVisible(true);
  ...
  protected void refreshGUI() {
    ...
    rmsLabel.setText(showCorrelation? 
    		ToolsRes.getString("DatasetCurveFitter.Label.CorrelationCoefficient"): //$NON-NLS-1$
    		ToolsRes.getString("DatasetCurveFitter.Label.RMSDeviation")); //$NON-NLS-1$
  ...
  protected FunctionTool getFitBuilder() {
    if(fitBuilder==null) {
      ...
      fitBuilder = new FunctionTool(null, new Component[] {newFitButton, cloneFitButton, deleteFitButton}) {
  ...
  /**
   * Gets the Pearson correlation R^2 between function and data
   */
  private double getPearsonCorrelationSquared(Function f, double[] x, double[] y) {
  	if (x.length<2) return 0;
    double mean_x = f.evaluate(x[0]);
    double mean_y = y[0];    
    for(int i=1; i<x.length; i++){
      mean_x += f.evaluate(x[i]);
      mean_y += y[i];
	  }
    mean_x /= x.length;
    mean_y /= x.length;
    ...
    for(int i=0; i<x.length; i++){
		  double delta_x = f.evaluate(x[i])-mean_x;
    ...
    double pop_sd_x = sum_sq_x/x.length;
    double pop_sd_y = sum_sq_y/x.length;
    double cov_x_y = sum_coproduct/x.length;
    return cov_x_y*cov_x_y/(pop_sd_x*pop_sd_y);
}
